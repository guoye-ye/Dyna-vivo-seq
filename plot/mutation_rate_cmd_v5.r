###### 此脚本和mutation_rate_cmd.R的区别：后者只能输入treated和untreated两个文件
###### 前者可以输入任意个数文件
...
ggplot(data=big_data, aes(x=type, y=rate,fill=group)) +
  ...
  theme(legend.text = element_text(face='plain',size=16), # legend内容大小
        legend.title = element_blank(),# legend标题大小
        #axis.line = element_line(size=1),   # 坐标轴线
        #text = element_text(family="Times"),# 文本字体
        axis.text.y = element_text(size = 16,face='plain',color='black'),# y轴标签样式
        axis.text.x = element_text(size = 16,face='plain',color='black',angle=0, hjust=0.5, vjust=0.5),        # x轴标签样式，angle=45 倾斜 45 度
        axis.title = element_text(size=20,face="plain"),  # 轴标题
        plot.title = element_text(hjust=0.5,size=22,face=2))   # 距，设置绘图区域距离边的据类，上、右、下、左
dev.off()
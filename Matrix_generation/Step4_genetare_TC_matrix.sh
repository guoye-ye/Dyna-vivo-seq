...
cd ${bam_dir}
perl /home/disk/lfh/pipeline/drop-seq/scripts/extract_digital_expression_matrix.pl ${sample}_star_gene_exon_tagged_TagIntronic_clean.TagTC.corrected.bam

############ 2. Generate T->C matrix with rds format
/usr/local/bin/Rscript  /home/disk/lfh/pipeline/drop-seq/scripts/Generate_T_C_matrix.R ${sample}_star_gene_exon_tagged_TagIntronic_clean.TagTC.corrected_gene_cell_UMI_read.txt ${num_core_barcode} ${sample}_TC_matrix.rds
